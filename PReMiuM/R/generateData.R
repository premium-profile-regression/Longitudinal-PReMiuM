...
generateSampleDataFile<-function(clusterSummary){
	...
	if(outcomeType=='MVN'){
		...
		Y <- matrix(0,ncol=nOutcomes,nrow=nSubjects)
	}
	if(nFixedEffects>0){
	...
	if(outcomeType == 'Longitudinal'){ ##//RJ generate longitudinal data
		...
		Yvalues <- unlist(as.vector(longY))
		IDvalues <- unlist(as.vector(longIDs))
	}
	...
	if(outcomeType == 'Longitudinal'){
		outLongData<-data.frame(cbind(IDvalues,Tvalues,Yvalues))
		longNames<-c('ID','time','outcome')
...
############################
# Sample datasets
clusSummaryCategoricalDiscrete<-function(){
	list(
...
clusSummaryMVNDiscrete<-function(nOutcomes=3){##//RJ summary generation function MVN
        list(
   'outcomeType'='MVN',
   'covariateType'='Discrete',
   'nCovariates'=5,
   ...
   'missingDataProb'=0,
   'nClusters'=2,
   'clusterSizes'=c(50,50),
   'includeCAR'=FALSE,
   'TauCAR'=100,
   'clusterData'=list(list('theta'=list('mu'=rep(10,nOutcomes),
			  	        			    'Sigma'=diag(nOutcomes)),
                           'covariateProbs'=list(c(0.8,0.1,0.1),
                                                 c(0.8,0.1,0.1),
                                                 c(0.8,0.1,0.1),
                                                 c(0.8,0.1,0.1),
                                                 c(0.8,0.1,0.1))),
                      list('theta'=list('mu'=seq(10,0,length=nOutcomes),
			  	        			    'Sigma'=diag(nOutcomes)),
                           'covariateProbs'=list(c(0.1,0.8,0.1),
                                                 c(0.1,0.8,0.1),
                                                 c(0.1,0.8,0.1),
