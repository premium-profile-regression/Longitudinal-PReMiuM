...
profRegr<-function(covNames, fixedEffectsNames, outcome="outcome", outcomeT=NA, data, longData=NULL,
  ...
  ##//RJ extra variation and longitudinal data mutually exclusive
  if (extraYVar==TRUE&(yModel=="Categorical"||yModel=="Normal"||yModel=="Survival"||yModel=="Longitudinal")) stop("Option extraYVar is only available for Bernoulli, Binomial and Poisson response.")
  ...
  if (kernel!="SQexponential" && kernel!="Quadratic") stop("Option kernelType for Gaussian Process should be either SQexponential or Quadratic.")
  ...
  for (k in 1:nCovariates){
    missingX<-is.na(dataMatrix[,(k+nOutcomes)])
    nMissingX<-sum(missingX)
    if (nMissingX>0) {
      dataMatrix[missingX,(k+nOutcomes)]<- rep(-999,nMissingX)
    }
  }
  ...
  ##//RJ add number of time points to file
  if(length(longData)>0){
    write(as.character(dim(longData)[1]),fileName,append=T,ncolumns=1)
  }else{
    write(as.character(nSubjects*nOutcomes), fileName,append=T,ncolumns=1)
  }
  if (xModel=="Mixed"){
  ...
  if (!missing(hyper)) {
    ...
    ##//RJ add L hypers to file
    if (!is.null(hyper$muLSignal)){
      write(paste("muL_signal=",hyper$muLSignal,sep=""),hyperFile,append=T)
    }
    if (!is.null(hyper$sigmaLSignal)){
      write(paste("sigmaL_signal=",hyper$sigmaLSignal,sep=""),hyperFile,append=T)
    }
    if (!is.null(hyper$muLLengthscale)){
      write(paste("muL_lengthscale=",hyper$muLLengthscale,sep=""),hyperFile,append=T)
    }
    if (!is.null(hyper$sigmaLLengthscale)){
      write(paste("sigmaL_lengthscale=",hyper$sigmaLLengthscale,sep=""),hyperFile,append=T)
    }
    if (!is.null(hyper$muLNoise)){
      write(paste("muL_noise=",hyper$muLNoise,sep=""),hyperFile,append=T)
    }
    if (!is.null(hyper$sigmaLNoise)){
    ...
    if (!is.null(hyper$sigmaLTime)){
      write(paste("sigmaL_time=",hyper$sigmaLTime,sep=""),hyperFile,append=T)
    }
    if (!is.null(hyper$MVNmu0)){
      write(paste("MVNmu0=",paste(hyper$MVNmu0,collapse=" ")," ",sep=""),hyperFile,append=T)
    }
    if (!is.null(hyper$MVNTau0)){
      write(paste("MVNTau0=",paste(t(hyper$MVNTau0),collapse=" ")," ",sep=""),hyperFile,append=T)
    }
    if (!is.null(hyper$MVNR0)){
      write(paste("MVNR0=",paste(t(hyper$MVNR0),collapse=" ")," ",sep=""),hyperFile,append=T)
    }
    if (!is.null(hyper$MVNkappa0)){
      write(paste("MVNkappa0=",hyper$MVNkappa0,sep=""),hyperFile,append=T)
    }
    if (!is.null(hyper$MVNnu0)){
      write(paste("MVNnu0=",hyper$MVNnu0,sep=""),hyperFile,append=T)
    }
  ...
  #if(includeResponse){
  yMat<-as.matrix(dataMatrix[,1:nOutcomes],ncol=nOutcomes)
  if(yModel=='Poisson'){
...
calcAvgRiskAndProfile<-function(clusObj,includeFixedEffects=F,proportionalHazards=F){
  ...
  if(includeResponse){
    ...
    if (yModel=="Survival"&&!weibullFixedShape){
      ...
      LArray<-array(0,dim=c(nSamples,nClusters,ifelse(kernel=="SQexponential",3,4)))
      riskArray<-array(0,dim=c(nSamples,nClusters,ifelse(kernel=="SQexponential",3,4)))
    } else if(yModel=="MVN"){
    ...
    if(includeResponse){
      ...
      if (yModel=="Categorical") {
        ...
        if (yModel=="Survival"&&!weibullFixedShape){
          ...
          currLVector<-scan(LFile,what=double(),skip=skipVal,n=ifelse(kernel=="SQexponential",3,4)*currMaxNClusters,quiet=T)
          currL<-matrix(currLVector,nrow=currMaxNClusters,ncol=ifelse(kernel=="SQexponential",3,4),byrow=T)
        } else if (yModel=="MVN"){
      ...
      for(c in 1:nClusters){
        ...
        if(yModel=="Poisson"){
          ...
          LArray[sweep-firstLine+1,c,]<-apply(matrix(currL[currZ[optAlloc[[c]]],],ncol=ifelse(kernel=="SQexponential",3,4)),2,mean)
...
plotRiskProfile<-function(riskProfObj,outFile,showRelativeRisk=F,orderBy=NULL,whichClusters=NULL,whichCovariates=NULL,
  ...
  if(!orderProvided){
    ...
    #meanSortIndex<-order(orderStat,decreasing=F) //AR
  ...
  if(yModel=='MVN'){
    ...
    for(j in 1:nOutcomes){
      ...
      plotObj<-plotObj+labs(title=outcome[j],plot.title=element_text(size=10))
      plotObj<-plotObj+
...
##//RJ functions to compute GP posterior in R
GP_cov <- function(times,L,kernelType){
  nTimes = length(times)
  ...
  for(i in 1:nTimes){
    if(kernelType=="SQexponential"){
      ...
      a=L[1]+L[2]*(times[i]-L[4])*(times[i]-L[4])
      Mat[i,i]=a*a + L[3]
    }
  }
  return(Mat)
...
calcPredictions<-function(riskProfObj,predictResponseFileName=NULL, doRaoBlackwell=F, fullSweepPredictions=F,fullSweepLogOR=F,fullSweepHazardRatio=F){
  ...
  ##//RJ
  if(yModel=='Longitudinal'){
    timepoints <- seq(min(longMat$time),max(longMat$time),length.out=20)
    thetaArray<-array(0,dim=c(nSamples,nPredictSubjects,length(timepoints)))
    varArray <- array(0,dim=c(nSamples,nPredictSubjects,length(timepoints)))
  }else if(yModel=="MVN"){
    thetaArray<-array(0,dim=c(nSamples,nPredictSubjects,nOutcomes))
    varArray <- array(0,dim=c(nSamples,nPredictSubjects,nOutcomes))
  }else{
  ...
  if(doRaoBlackwell){
    ...
    for(sweep in firstLine:lastLine){
      ...
      if (yModel=="Categorical") {
        ...
        currThetaVector<-scan(LFile,what=double(),skip=skipVal,n=currNClusters*ifelse(kernel=="SQexponential",3,4),quiet=T)
        params<-matrix(currThetaVector,ncol=ifelse(kernel=="SQexponential",3,4),byrow=T)
        currTheta <- matrix(0,ncol=length(timepoints),nrow=dim(params)[1])
        currVar <- matrix(0,ncol=length(timepoints),nrow=dim(params)[1])
        ...
        currThetaVector<-scan(MVNmuFile,what=double(),skip=skipVal,n=currNClusters*nOutcomes,quiet=T)
        params<-matrix(currThetaVector,ncol=nOutcomes,byrow=T)
        currVarVector<-scan(MVNSigmaFile,what=double(),skip=skipVal,n=currNClusters*(nOutcomes*(nOutcomes+1)/2),quiet=T)
        var<-matrix(currVarVector,ncol=(nOutcomes*(nOutcomes+1)/2),byrow=T)
        currTheta <- matrix(0,ncol=nOutcomes,nrow=dim(params)[1])
        currVar <- matrix(0,ncol=nOutcomes,nrow=dim(params)[1])
        for(c in 1:dim(params)[1]){
      ...
      thetaArray[sweep-firstLine+1,,]<-as.matrix(currTheta[currZ,],ncol=dim(thetaArray)[3])
      if(yModel=='Longitudinal'||yModel=="MVN"){
        varArray[sweep-firstLine+1,,]<-as.matrix(currVar[currZ,],ncol=dim(thetaArray)[3])
      }
  ...
  if(yModel=='Longitudinal'){
    predictedY<-array(0,dim=c(nSamples,nPredictSubjects,length(timepoints)))
    predictedYVar<-array(0,dim=c(nSamples,nPredictSubjects,length(timepoints)))
  }else if(yModel=='MVN'){
    predictedY<-array(0,dim=c(nSamples,nPredictSubjects,nOutcomes))
    predictedYVar<-array(0,dim=c(nSamples,nPredictSubjects,nOutcomes))
  }else{
  ...
  for(sweep in 1:nSamples){
    ...
    }else if(yModel=="Longitudinal"||yModel=="MVN"){
      predictedY[sweep,,]<-lambda
      predictedYVar[sweep,,]<-varArray[sweep,,]
    }
  }
  #print(predictedY[,1,])
  if(responseProvided){
  ...
  if(yModel=='Longitudinal'){
    ...
    output$predictedYVar<-apply(predictedYVar,c(2,3),median)
  }
  if(yModel=="MVN"){
    output$predictedYVarPerSweep<-predictedYVar
    output$predictedYVar<-apply(predictedYVar,c(2,3),median)
  }
  if(fullSweepLogOR){
...
setHyperparams<-function(shapeAlpha=NULL,rateAlpha=NULL,aPhi=NULL,mu0=NULL,Tau0=NULL,R0=NULL,                         kappa0=NULL,nu0=NULL,muTheta=NULL,sigmaTheta=NULL,dofTheta=NULL,muBeta=NULL,sigmaBeta=NULL,dofBeta=NULL,shapeTauEpsilon=NULL,rateTauEpsilon=NULL,aRho=NULL,bRho=NULL,atomRho=NULL,shapeSigmaSqY=NULL,scaleSigmaSqY=NULL,                     rSlice=NULL,truncationEps=NULL,shapeTauCAR=NULL,rateTauCAR=NULL,shapeNu=NULL,scaleNu=NULL,initAlloc=NULL,                       muLSignal=0,sigmaLSignal=0,muLLengthscale=0,sigmaLLengthscale=0,muLNoise=0,sigmaLNoise=0,MVNmu0=NULL,MVNTau0=NULL,MVNR0=NULL,                         MVNkappa0=NULL,MVNnu0=NULL){
  ...
  if (!is.null(muLSignal)){
    out$muLSignal<-muLSignal
  }
  if (!is.null(sigmaLSignal)){
    out$sigmaLSignal<-sigmaLSignal
  }
  if (!is.null(muLLengthscale)){
    out$muLLengthscale<-muLLengthscale
  }
  if (!is.null(sigmaLLengthscale)){
    out$sigmaLLengthscale<-sigmaLLengthscale
  }
  if (!is.null(muLNoise)){
    out$muLNoise<-muLNoise
  }
  if (!is.null(sigmaLNoise)){
    out$sigmaLNoise<-sigmaLNoise
  }
  if (!is.null(MVNmu0)){
    out$MVNmu0<-MVNmu0
  }
  ...
  if (!is.null(MVNkappa0)){
    out$MVNkappa0<-MVNkappa0
  }
  if (!is.null(MVNnu0)){
    out$MVNnu0<-MVNnu0
  }
...
plotPredictions<-function(outfile="condDensity.pdf",runInfoObj,predictions,logOR=FALSE){
  ...
  for (i in 1:length(runInfoObj)) assign(names(runInfoObj)[i],runInfoObj[[i]])
  if (yModel!="Longitudinal"&&yModel!="Bernoulli"&&yModel!="Normal"&&yModel!="Survival"&&yModel!="MVN") stop("This function has been developed for Bernoulli, Normal, Survival, MVN and longitudinal responses only.")
  ...
  if(yModel=='Longitudinal'){##//RJ
    ...
    plotObj <- plotObj + geom_line(aes(x=time,y=score,group=Subject,colour=as.factor(Subject)),size=2)
    plotObj <- plotObj + geom_line(aes(x=time,y=score+stdev,group=Subject,colour=as.factor(Subject)),size=1)
    plotObj <- plotObj + geom_line(aes(x=time,y=score-stdev,group=Subject,colour=as.factor(Subject)),size=1)
    plotObj <- plotObj + theme(legend.title=element_text(size=20)) + theme(legend.text=element_text(size=15))
    plotObj <- plotObj + labs(colour="Subject")
    #plotObj<-plotObj+theme(legend.position="none")
    plotObj <- plotObj + labs(y="Score\n")+theme(axis.title.y=element_text(size=20,angle=90))
    plotObj <- plotObj + labs(x="\nTime")+theme(axis.title.x=element_text(size=20))
    plotObj <- plotObj + theme(axis.text.x=element_text(size=15)) + theme(axis.text.y=element_text(size=15))
    plotObj <- plotObj + coord_cartesian(ylim = c(min(plotDF$score-plotDF$stdev),max(plotDF$score+plotDF$stdev)))
    print(plotObj)
    ...
    for(j in 1:nOutcomes){
      ...
      plotObj<-plotObj+geom_point(aes(x=as.factor(Subject),y=lowerMu,colour=as.factor(Subject)),size=1.5)
      plotObj<-plotObj+geom_point(aes(x=as.factor(Subject),y=upperMu,colour=as.factor(Subject)),size=1.5)
      plotObj<-plotObj+
        theme(legend.position="none")+labs(x="Subject")+theme(axis.title.x=element_text(size=10))
      if(j==1){
        plotObj<-plotObj+labs(y="Mean")+theme(axis.title.y=element_text(size=10,angle=90))
      }else{
        plotObj<-plotObj+theme(axis.title.y=element_blank())
      ...
      # Plot the variances
      sigmaMat<-sqrt(predictions$predictedYVarPerSweep[,,j])
      sigmaMeans<-apply(sigmaMat,2,mean)
      sigmaMean<-mean(sigmaMeans)
      sigmaLower<-apply(sigmaMat,2,quantile,0.05)
      sigmaUpper<-apply(sigmaMat,2,quantile,0.95)
      ...
      plotObj<-plotObj+geom_boxplot(aes(x=as.factor(Subject),y=sigma,fill=as.factor(Subject)),outlier.size=0.5)
      plotObj<-plotObj+geom_point(aes(x=as.factor(Subject),y=lowerSigma,colour=as.factor(Subject)),size=1.5)
      plotObj<-plotObj+geom_point(aes(x=as.factor(Subject),y=upperSigma,colour=as.factor(Subject)),size=1.5)
      plotObj<-plotObj+
        theme(legend.position="none")+labs(x="Subject")+theme(axis.title.x=element_text(size=10))
      if(j==1){
        plotObj<-plotObj+labs(y="Std Dev")+theme(axis.title.y=element_text(size=10,angle=90))
